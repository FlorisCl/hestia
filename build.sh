...
docker build --build-arg=APP_VERSION="$APP_VERSION" --tag floriscl/hestia-bot:$TAG -f docker/Dockerfile.bot .
docker build --build-arg=APP_VERSION="$APP_VERSION" --tag floriscl/hestia-scraper:$TAG -f docker/Dockerfile.scraper .
docker build --build-arg=APP_VERSION="$APP_VERSION" --build-arg=APP_VERSION_DATE="$(git show -s --format=%cs HEAD)" --tag floris.cl/hestia-web:$TAG -f web/Dockerfile web/
